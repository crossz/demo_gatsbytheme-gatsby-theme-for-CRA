...
const useStyles = makeStyles((theme) => ({
  ...
  dot: {
    ...
    position: 'absolute',
    // top: theme.spacing(0.5),
    width: (props) => theme.spacing(props.matches ? 1 : props.size ? props.size : 1.75),
    height: (props) => theme.spacing(props.matches ? 1 : props.size ? props.size : 1.75),
...
const TitleDot = ({ bgcolor, size, left }) => {
  ...
  return (
    <Box className={classes.root}>
      <Box className={classes.dot} bgcolor={bgcolor || 'secondary.main'} />
    </Box>
  )
}
